"""
脚本名称: region_sector_fund_flow.py
功能: 抓取东方财富网地域板块资金流向排行（今日、5日、10日）
数据命名: 板块资金流-地域资金流_周期_YYYY-MM-DD.csv
"""
...
def fetch_region_data(period_name, config):
    """获取指定周期的地域板块数据"""
    ...
    try:
        ...
        if data_json.get('rc') == 0 and data_json.get('data'):
            ...
            logging.info(f"{period_name} 成功获取 {len(items)} 条地域板块数据")
            return items
        else:
...
def parse_region_data(items, field_map):
    """根据字段映射解析数据"""
    parsed = []
    for item in items:
        try:
...
def main():
    ...
    for period_name, config in REGION_CONFIGS.items():
        # 文件名格式：板块资金流-地域资金流_周期_日期.csv
        filename = f"板块资金流-地域资金流_{period_name}_{today}.csv"
        ...
        logging.info(f"开始抓取地域板块 {period_name} 排行数据...")
        items = fetch_region_data(period_name, config)

        if items:
            data = parse_region_data(items, config['field_map'])
            if data:
                print(f"\n{period_name} 地域排行前3条预览：")
